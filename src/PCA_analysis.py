...
def reshape_for_PCA(data, mask_filename, normalize=False, saturation_threshold=None):
    ...
    mask_binned = block_reduce(
        mask, block_size=(binning, binning), func=np.mean)
    mask_binned = (mask_binned > 0.9).astype(np.uint8)

    print("Reshaping data for PCA")
    applied = data.apply(lambda row: aux_reshape_for_PCA(
        row['file name'], mask_binned, normalize=normalize, saturation_threshold=saturation_threshold), axis=1, result_type='expand')
...
def PCA_analysis(data, n_components, PCA_variable='I', option="average"):
    ...
    Number_of_replicates = np.cumsum(
        [len(item) for item in All_images_list])[:-1]
    ...
    # np.split(Images_In_PCA_space, Number_of_replicates, axis=0)
    PCA_coefficients = list(Images_In_PCA_space)
    ...
    for i in data.index:
        data.at[i, PCA_variable +
                '_PCA_eigenvectors'] = np.vstack([Eigenvector0, PCA_components])
...
def plot_PCA_2D(data, comp1, comp2, PCA_variable='I', hue='construct', figsize=(14, 20), reverse_x=False, reverse_y=False):
    ...
    for i, row in data.iterrows():
        ...
        data.loc[i, 'PCA_a'] = x
        data.loc[i, 'PCA_b'] = y
        Eigen0 = coo_matrix((row[PCA_variable+'_PCA_eigenvectors']
                            [0, :], (row['x'], row['y']))).toarray()  # [200:,100:]
        Eigen1 = coo_matrix((row[PCA_variable+'_PCA_eigenvectors']
                            [comp1, :], (row['x'], row['y']))).toarray()  # [200:,100:]
        Eigen2 = coo_matrix((row[PCA_variable+'_PCA_eigenvectors']
                            [comp2, :], (row['x'], row['y']))).toarray()  # [200:,100:]
...
def plot_rotate_PCA_2D(data, comp1, comp2, PCA_variable='freq', hue='construct', v1_x=1, v1_y=0, v2_x=0, v2_y=1, origin="empty", figsize=(14, 20), cmap="jet"):
    ...
    markers = ['o', '+', 'x', '*']
    markers_cycle = [markers[i % len(markers)] for i in range(Num_colors)]
    custom_cycler = (cycler(color=sns.color_palette(
        "husl", Num_colors), marker=markers_cycle))
    #f_ax1.set_prop_cycle(custom_cycler)
    colors = custom_cycler.by_key()["color"]
    markers = custom_cycler.by_key()["marker"]
    ...
    x_origin = np.mean(np.stack(
        data.loc[data["construct"] == origin][PCA_variable+'_PCA_coefficients'].values)[:, comp1-1])
    y_origin = np.mean(np.stack(
        data.loc[data["construct"] == origin][PCA_variable+'_PCA_coefficients'].values)[:, comp2-1])
...
if __name__ == '__main__':
    ...
    variable = "I"
    dataframe = PCA_analysis(dataframe, n_components=4,
                             PCA_variable=variable, option="all")
    dataframe = dataframe.reset_index()
    plot_rotate_PCA_2D(dataframe, comp1=1, comp2=2, PCA_variable=variable,
                       v1_x=-1, v1_y=0, v2_x=0, v2_y=-1, origin="empty", figsize=(10, 8))
